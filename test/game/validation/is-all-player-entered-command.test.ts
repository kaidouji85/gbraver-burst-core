import { BatteryCommand, EMPTY_PLAYER, Player, PlayerCommand } from "../../../src";
...
test("全プレイヤーがコマンド入力したことを正しく検知できる", () => {
  ...
  const commands: [PlayerCommand, PlayerCommand] = [player1Command, player2Command];
...
test("1人のプレイヤーしかコマンド入力していない場合、全プレイヤー入力でないと判定する", () => {
  const players: [Player, Player] = [player1, player2];
  const commands: [PlayerCommand, PlayerCommand] = [player1Command, player1Command];
  const result = isAllPlayerEnteredCommand(players, commands);
