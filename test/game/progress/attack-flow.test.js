...
test('バッテリー宣言から攻撃までを正しく進めることができる', () => {
  ...
  const attackerBattery = {playerId: attacker.playerId, command: {type: 'BATTERY_COMMAND', battery: 3}};
  const defender = {...EMPTY_PLAYER_STATE, playerId: 'player2'};
  const defenderBattery = {playerId: defender.playerId, command: {type: 'BATTERY_COMMAND', battery: 2}};
  const lastState = {...EMPTY_GAME_STATE, players: [attacker, defender], activePlayerId: attacker.playerId};
