...
test("戦闘後のコマンド入力フェイズが正しく適用される", () => {
  ...
  const player02 = { ...EMPTY_PLAYER_STATE,
    playerId: "player02",
    armdozer: { ...EMPTY_ARMDOZER_STATE,
      battery: 3,
      maxBattery: 5,
  ...
  const result = inputCommand({ ...EMPTY_GAME_STATE,
...
test("効果適用フロー後のコマンド入力フェイズ効果が正しく処理される", () => {
  ...
  const snapShotPath = path.join(__dirname, "input-command__after-effect-activation.json");
  shouldUpdateSnapShot() && exportSnapShotJSON(snapShotPath, result);
  const snapShot = shouldUpdateSnapShot() ? result : importSnapShotJSON(snapShotPath);
  expect(result).toEqual(snapShot);
});