...
export function turnChange(lastState: GameState): GameStateX<TurnChange> {
  ...
  const updatedBattery = turnChangeRecoverBattery(nextActivePlayer.armdozer.battery, nextActivePlayer.armdozer.maxBattery, BATTERY_RECOVERY_VALUE);
  const updatedPlayer = { ...nextActivePlayer,
    armdozer: { ...nextActivePlayer.armdozer,
      battery: updatedBattery
    }
  };
  const updatedPlayerList = lastState.players.map(v => v.playerId === updatedPlayer.playerId ? updatedPlayer : v);
  return { ...lastState,
